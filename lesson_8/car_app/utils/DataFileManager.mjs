...
class DataFileManager {
    ...
    updateItemById(id, updatedProperties) {
        try {
            ...
            if (index !== -1) {
                // Оновлюємо тільки ті властивості, які не є порожніми
                data[index] = {
                    ...data[index],
                    ...Object.keys(updatedProperties).reduce((acc, key) => {
                        // Якщо властивість пуста (undefined, null або порожній рядок), зберігаємо старе значення
                        acc[key] = updatedProperties[key] !== undefined && updatedProperties[key] !== null && updatedProperties[key] !== ''
