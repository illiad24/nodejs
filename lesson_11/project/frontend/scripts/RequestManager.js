class RequestManager {
    ...
    static async fetchData(url, addAuthorization = true) {
        ...
        try {
            ...
            response = await fetch(RequestManager.getServerRoute(url), {
                method: 'GET',
                headers: headers,
            })
    ...
    //   Метод для додавання кнопки додати продукт
    static showAddButton(addAuthorization = true) {
        const addButtons = document.querySelectorAll('.js-add-link');
        if (addButtons) {
            addButtons.forEach(addButton => {
                if (addAuthorization && RequestManager.isAuthenticated())
                    addButton.classList.remove('dn')
                else
